...
class ColombianLocationRecognizer(PatternRecognizer):
    ...
    _LOCATIONS = {
        "ADDRESS": {
            ...
            "keywords": ["calle", "carrera", "avenida", "transversal", "diagonal", "cra", "av", "cl"],
            "pattern": r"(?:calle|carrera|avenida|transversal|diagonal|cra?|av|cl)\s+\d{1,3}[a-z]?\s*#?\s*\d{1,3}[a-z]?[-–]\d{1,3}[a-z]?(?:\s+(?:apto?|apartamento|casa|interior|local|oficina|piso|torre)\s*[a-z0-9-]+)?",
            "score": 0.95
        },
        "CITY": {
            "name": "Ciudad",
