from presidio_analyzer import AnalyzerEngine
from presidio_anonymizer import AnonymizerEngine
from typing import List, Dict, Any, Optional, Union
import logging

class PresidioService:
    def __init__(self):
        self.analyzer = AnalyzerEngine()
        self.anonymizer = AnonymizerEngine()
        self.logger = logging.getLogger(__name__)
    
    def analyze_text(self, text: str, language: str = 'en') -> List[Dict[str, Any]]:
        """Analiza texto y retorna entidades encontradas"""
        try:
            results = self.analyzer.analyze(text=text, language=language)
            return [{
                'entity_type': r.entity_type,
                'start': r.start,
                'end': r.end,
                'score': r.score
            } for r in results]
        except Exception as e:
            self.logger.error(f"Error en analyze_text: {str(e)}")
            return []
    
    def anonymize_text(self, text: str, language: str = 'en') -> str:
        """Anonimiza texto reemplazando entidades sensibles"""
        try:
            results = self.analyzer.analyze(text=text, language=language)
            anonymized = self.anonymizer.anonymize(text=text, analyzer_results=results)
            return anonymized.text
        except Exception as e:
            self.logger.error(f"Error en anonymize_text: {str(e)}")
            return text
            
    def filter_entities(self, 
                        text: str, 
                        entity_types: Optional[Union[List[str], str]] = None, 
                        min_score: float = 0.0, 
                        language: str = 'en') -> Dict[str, Any]:
        """
        Filtra las entidades por tipo y puntaje mínimo, y puede anonimizar el texto si se solicita.
        
        Args:
            text: Texto a analizar
            entity_types: Tipo(s) de entidad a filtrar (p.ej., 'PERSON', 'EMAIL_ADDRESS', etc.)
                         Si es None, incluye todas las entidades.
            min_score: Puntaje mínimo para filtrar entidades (0.0 a 1.0)
            language: Idioma del texto. Por defecto 'en'.
            
        Returns:
            Diccionario con el texto original, entidades filtradas y texto anonimizado
        """
        # Verificar los tipos de entidad solicitados y asegurar que estén en formato correcto
        available_entity_types = [
            "PERSON", "EMAIL_ADDRESS", "PHONE_NUMBER", "CREDIT_CARD", "URL", 
            "LOCATION", "DATE_TIME", "NRP", "ORGANIZATION", "US_SSN", 
            "US_ITIN", "US_DRIVER_LICENSE", "US_BANK_NUMBER", "US_PASSPORT"
        ]
        
        try:
            # Convertir entity_types a lista si es una cadena
            if entity_types and isinstance(entity_types, str):
                entity_types = [entity_types]
            
            # Validar que los tipos de entidad solicitados sean válidos
            if entity_types:
                valid_entity_types = []
                for entity_type in entity_types:
                    if entity_type in available_entity_types:
                        valid_entity_types.append(entity_type)
                
                # Si no hay tipos válidos, incluir todos los disponibles
                if not valid_entity_types:
                    entity_types = None
                else:
                    entity_types = valid_entity_types
                    
            # Obtener todas las entidades
            all_entities = self.analyze_text(text=text, language=language)
            
            # Filtrar por tipo de entidad y puntaje
            filtered_entities = []
            for entity in all_entities:
                if (entity_types is None or entity['entity_type'] in entity_types) and entity['score'] >= min_score:
                    filtered_entities.append(entity)
            
            # Extraer los fragmentos de texto correspondientes a cada entidad filtrada
            entity_texts = []
            for entity in filtered_entities:
                start, end = entity['start'], entity['end']
                entity_texts.append({
                    'entity_type': entity['entity_type'],
                    'score': entity['score'],
                    'text': text[start:end]
                })
                
            # Anonimizar solo las entidades filtradas si existen
            anonymized_text = ""
            if filtered_entities:
                try:
                    # Ejecutar el análisis nuevamente y filtrar los resultados originales
                    # en lugar de recrear los objetos RecognizerResult
                    all_results = self.analyzer.analyze(text=text, language=language)
                    
                    # Filtrar los resultados originales basados en las entidades filtradas
                    filtered_results = []
                    for result in all_results:
                        for entity in filtered_entities:
                            if (result.entity_type == entity['entity_type'] and 
                                result.start == entity['start'] and 
                                result.end == entity['end']):
                                filtered_results.append(result)
                                break
                    
                    if filtered_results:
                        anonymized = self.anonymizer.anonymize(text=text, analyzer_results=filtered_results)
                        anonymized_text = anonymized.text
                    else:
                        anonymized_text = text
                except Exception as e:
                    self.logger.error(f"Error al anonimizar con entidades filtradas: {str(e)}")
                    anonymized_text = text
            else:
                anonymized_text = text  # No hay cambios si no hay entidades filtradas
            
            # Construir respuesta
            return {
                'original_text': text,
                'filtered_entities': filtered_entities,
                'entity_texts': entity_texts,
                'anonymized_text': anonymized_text
            }
        except Exception as e:
            self.logger.error(f"Error en filter_entities: {str(e)}")
            # Devolver una respuesta de error más descriptiva
            return {
                'original_text': text,
                'error': str(e),
                'filtered_entities': [],
                'entity_texts': [],
                'anonymized_text': text
            }