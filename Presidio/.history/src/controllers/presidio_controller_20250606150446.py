...
class PresidioController:
    ...
    def preview_anonymization(self):
        """Endpoint para previsualizar la anonimización"""
        self.logger.info("Iniciando previsualización de anonimización")
        try:
            data = request.json
            text = data['text']
            self.logger.info(f"Previsualizando anonimización para texto de {len(text)} caracteres")
            
            # Obtener las configuraciones actuales de anonimización
            current_anonymization_settings = self.presidio_service.get_current_anonymization_settings()
            
            # Realizar la anonimización con las configuraciones actuales
            anonymized_text = self.presidio_service.anonymize_text(text)
            
            response = {
                'anonymized_text': anonymized_text,
                'current_settings': current_anonymization_settings
            }
            
            self.logger.info("Previsualización completada exitosamente")
            return jsonify(response)
        except Exception as e:
            self.logger.error(f"Error en previsualización de anonimización: {str(e)}")
            return jsonify({'error': str(e)}), 500
