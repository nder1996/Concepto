...
class PresidioController:
    ...
    def anonymize_selective_endpoint(self):
        ...
        try:
            ...
            self.logger.info(f"Anonimizando selectivamente texto de {len(text)} caracteres (idioma: {language})")
            
            # Realizar la anonimización selectiva
            anonymized_text = self.presidio_service.anonymize_selective(
                text=text,
                language=language
            )
            
            self.logger.info("Anonimización selectiva completada")
            
            # Devolver el resultado
            return jsonify({
                'original_text': text,
                'anonymized_text': anonymized_text,
                'method': 'anonymize_selective',
                'entities_considered': self.presidio_service.target_entities,
                'score_threshold': self.presidio_service.score_threshold
            })
            
        except Exception as e:
            self.logger.error(f"Error en anonimización selectiva: {str(e)}")
            return jsonify({
                'error': f"Error en la anonimización selectiva: {str(e)}",
    ...
    def anonymize_selective_file(self):
        ...
        try:
            ...
            self.logger.info(f"Procesando archivo para anonimización selectiva: {file.filename}")
            ...
            self.logger.info("Anonimización selectiva de archivo completada")
            ...
            self.logger.error(f"Error en anonimización selectiva de archivo: {str(e)}")
            return jsonify({
                'error': f"Error en la anonimización selectiva de archivo: {str(e)}",
                'suggestion': 'Verifica que el formato del archivo sea compatible y que el idioma esté correctamente especificado'
            }), 500