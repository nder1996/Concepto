...
class PresidioController:
    ...
    def register_routes(self, app):
        ...
        app.add_url_rule('/filter-entities', 'filter_entities', self.filter_entities, methods=['POST'])
        app.add_url_rule('/filter-entities-file', 'filter_entities_file', self.filter_entities_file, methods=['POST'])
    ...
    def filter_entities(self):
        """Endpoint para filtrar entidades específicas con un puntaje mínimo"""
        self.logger.info("Iniciando filtrado de entidades en texto")
        try:
            data = request.json
            text = data['text']
            ...
            self.logger.info(f"Filtrando entidades en texto de {len(text)} caracteres")
            self.logger.info(f"Tipos de entidades: {entity_types}, Puntaje mínimo: {min_score}")
            
            results = self.presidio_service.filter_entities(
                text=text,
            ...
            self.logger.info(f"Filtrado completado: {len(results['filtered_entities'])} entidades encontradas")
            
            return jsonify(results)
        except Exception as e:
            self.logger.error(f"Error en filtrado de entidades: {str(e)}")
            return jsonify({'error': str(e)}), 500
    
    def filter_entities_file(self):
        """Endpoint para filtrar entidades específicas con un puntaje mínimo en archivos"""
        self.logger.info("Iniciando filtrado de entidades en archivo")
        try:
            if 'file' not in request.files:
                return jsonify({'error': 'No se proporcionó archivo'}), 400
            ...
            entity_types = form_data.get('entity_types', None)
            if entity_types:
                ...
                try:
                    ...
                    # Si no es un JSON válido, tratarlo como una sola entidad
                    entity_types = [entity_types]
            
            min_score = float(form_data.get('min_score', 0.0))
            language = form_data.get('language', 'en')
            
            self.logger.info(f"Procesando archivo para filtrado: {file.filename}")
            
            # Extraer texto del archivo
            extracted_text = self.file_processor.process_file(file_content, file.filename)
            self.logger.info(f"Texto extraído: {len(extracted_text)} caracteres")
            
            # Filtrar entidades en el texto extraído
            results = self.presidio_service.filter_entities(
                text=extracted_text, 
                entity_types=entity_types,
                min_score=min_score,
            ...
            self.logger.info(f"Filtrado completado en archivo: {len(results['filtered_entities'])} entidades encontradas")
            
            # Añadir el nombre del archivo a la respuesta
            results['filename'] = file.filename
            
            return jsonify(results)
        except Exception as e:
            self.logger.error(f"Error filtrando entidades en archivo: {str(e)}")
            return jsonify({'error': str(e)}), 500