...
class PresidioService:
    ...
    def analyze_text(self, text: str, language: str = 'es') -> List[Dict[str, Any]]:
        ...
        filtered_results = []
        for r in results:
            # Comprobar si la entidad es una entidad objetivo o una variante de COLOMBIAN_ID_DOC
            is_target = r.entity_type in self.target_entities
            is_colombian_doc = r.entity_type.startswith("COLOMBIAN_ID_DOC")
            
            # Determinar el umbral apropiado
            if is_colombian_doc:
                threshold = thresholds.get("COLOMBIAN_ID_DOC", 0.1)
            else:
                threshold = thresholds.get(r.entity_type, 0.80)
            
            # Incluir la entidad si cumple los criterios
            if (is_target or is_colombian_doc) and r.score >= threshold:
